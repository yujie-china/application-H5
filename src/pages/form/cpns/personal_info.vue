    ...
    <div class="application_info">
        ...
        <van-form :model="FormData" label-align="top">
            <van-cell-group inset class="flex-container">
                <van-row>
                    <van-col span="12">
                        <van-field v-model="FormData.cn_name" maxlength="11" label="中文名" :rules="cnameRules" placeholder="请输入中文名" required />
                    </van-col>
                    <van-col span="12">
                        <van-field v-model="FormData.eng_name" maxlength="11" label="英文名" :rules="enameRules" placeholder="请输入英文名" required />
                    </van-col>
                </van-row>
                ...
                <van-popup v-model:show="locationShow" round position="bottom">
                    <van-cascader v-model="locationCascaderValue" title="请选择所在地区" :options="locationOptions"
                ...
                <van-popup v-model:show="livingShow" round position="bottom">
                    <van-cascader v-model="livingCascaderValue" title="请选择所在地区" :options="livingOptions"
                ...
                <van-col span="12">
                    <van-field v-model="FormData.nation" :rules="nationRules" maxlength="11" label="民族" placeholder="请输入民族" required />
                </van-col>

                <van-field v-model="FormData.education_level" :rules="edulevelRules" maxlength="11" label="最高学历" placeholder="请输入最高学历" required />
                <van-row>
                    <van-col span="12">
                        <van-field v-model="FormData.education_school" :rules="snameRules" maxlength="24" label="毕业学校" placeholder="请输入毕业学校" required />
                    </van-col>
                    <van-col span="12">
                        <van-field v-model="FormData.education_major" :rules="majRules" maxlength="11" label="毕业专业" placeholder="请输入毕业专业" required />
                    </van-col>
                </van-row>
                <van-field v-model="FormData.phone" :rules="mobileRules" label="手机号" placeholder="请输入手机号码" maxlength="11" required />
                <van-field v-model="FormData.wx" :rules="wxRules" label="微信号" placeholder="请输入微信号" required />
                <van-field v-model="FormData.ID_card" :rules="IDcardRules" label="身份证号码" placeholder="请输入身份证号码" maxlength="18" required />
                <van-field v-model="FormData.email" :rules="emailRules" label="常用邮箱（录用通知使用）" placeholder="请输入常用邮箱" required />
                <van-field v-model="FormData.emergency_phone" :rules="mobileRules" label="紧急联系人电话" placeholder="请输入紧急联系人电话" maxlength="11"
                    required />
                <van-field v-model="FormData.sick" :rules="helRules" label="健康状况" placeholder=" 极好 很好 好 一般 差 " maxlength="2" required />
                <van-field v-model="FormData.sick_level" :rules="healRules" maxlength="36" label="是否患有影响本职工作的各种疾病（如：遗传病、慢性病、传染病等）以及需要休息的各种身体状态，如有，请列明"
                    placeholder="请如实告知" required />
                ...
                <van-field v-model="FormData.home_address" :rules="addressRules" label="家庭住址" placeholder="请输入家庭住址" required/>
                <van-field v-model="FormData.habit" :rules="hobbyRules" maxlength="11" label="兴趣爱好" placeholder="请输入兴趣爱好" required/>
            </van-cell-group>
        </van-form>
    </div>
...
import {
  addressRules,
  cnameRules, edulevelRules,
  emailRules,
  enameRules,
  healRules,
  helRules,
  hobbyRules, IDcardRules, majRules,
  mobileRules, nationRules, snameRules, wxRules
} from "@/utils/rules.ts";
